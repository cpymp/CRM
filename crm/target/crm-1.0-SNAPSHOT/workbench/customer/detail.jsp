			...
			$("#saveRemarkButton").click(function (){
				...
				$.ajax({
					...
					success:function (data){
						/*
								data:  remark对象？？
									["success":true,"remark":{备注}]
						 */
						var html = "";
						if (data.success){
							//成功过后，在页表下插入新的div框框展现备注。
							html += '<div  id="d'+data.remark.id+'" class="remarkDiv" style="height: 60px;">';
							html += '<img title="${customer.name}" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
							html += '<div style="position: relative; top: -40px; left: 40px;" >';
							html += '<h5>'+data.remark.noteContent+'</h5>';
							html += '<font color="gray">联系人</font> <font color="gray">-</font> <b>(暂无)先生-${customer.name}</b> <small style="color: gray; id="e'+data.remark.id+'" ">'+data.remark.createTime+'由'+data.remark.createBy+'</small>';
							html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
							html += '<a onclick="editRemark(\''+data.remark.id+'\')" class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>';
							html += '&nbsp;&nbsp;&nbsp;&nbsp;';
							html += '<a onclick="deleteRemark(\''+data.remark.id+'\')" class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #FF0000;"></span></a>';
							html += '</div>';
							html += '</div>';
							html += '</div>';
				...
				$.ajax({
					...
					success:function (data){
						...
						if (data.success){
							...
							$("#e"+id).html(data.customer.noteContent);
							$("#s"+id).html(data.customer.editTime+"由"+data.customer.editBy);
		...
		function showRemarkList(){
			...
			$.ajax({
				...
				success:function (data){
					/*
							data:
								{,"remarkList":[{remark1},{remark1},{remark1}]}

					 */

					//成功后返回 Remark 列表  ， 根据remark列表的值，在框框中生成remaList
					if(data.success){
						$("#remark").val("");
							var html = "";
						$.each(data.remarkList,function (i,n){
							html += '<div  id="x'+n.id+'" class="remarkDiv" style="height: 60px;">';
							html += '<img title="${customer.name}" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
							html += '<div style="position: relative; top: -40px; left: 40px;" >';
							html += '<h5 id="e'+n.id+'">'+n.noteContent+'</h5>';
							html += '<font color="gray">联系人</font> <font color="gray">-</font> <b>(暂无)先生-${customer.name}</b> <small style="color: gray;" id="s'+n.id+'">'+(n.editFlag==0?n.createTime:n.editTime)+'由'+(n.editFlag == 0?n.createBy:n.createTime)+'</small>';
							html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
							...
							html += '&nbsp;&nbsp;&nbsp;&nbsp;';
							html += '<a onclick="deleteRemark(\''+n.id+'\')" class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color:#FF0000;"></span></a>';
							html += '</div>';
							html += '</div>';
							html += '</div>';
						})
							$("#customerRemarkBody").html(html);
						$("#customerRemarkBody").on("mouseover",".remarkDiv",function(){
							$(this).children("div").children("div").show();
						})
						$("#customerRemarkBody").on("mouseout",".remarkDiv",function(){
							$(this).children("div").children("div").hide();
						})
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-horizontal" role="form">
                        ...
                        <div style="position: relative;top: 15px;">
                            <div class="form-group">
                                <label for="edit-updateContactSummary" class="col-sm-2 control-label">联系纪要</label>
                                <div class="col-sm-10" style="width: 81%;">
                                    <textarea class="form-control" rows="3" id="edit-updateContactSummary"></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="edit-updateNextContactTime" class="col-sm-2 control-label">下次联系时间</label>
                                <div class="col-sm-10" style="width: 300px;">
                                    <input type="text" class="form-control" id="edit-updateNextContactTime">
                                </div>
                            </div>
                        </div>
		...
		<div id="remarkDiv" style="background-color: #E6E6E6; width: 870px; height: 90px;">
			<form role="form" style="position: relative;top: 10px; left: 10px;">
				...
				<p id="cancelAndSaveBtn" style="position: relative;left: 737px; top: 10px; display: none;">
					...
					<button type="button" class="btn btn-primary" id="saveRemarkButton">保存</button>
				</p>
			</form>
		</div>
	</div>
